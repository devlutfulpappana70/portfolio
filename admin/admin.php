<?php
namespace AestheticaSpace\Admin;

use AestheticaSpace\Admin\Installer\Installer;
use AestheticaSpace\Admin\Options\Options_Manager;
use AestheticaSpace\Admin\Upgrader\Upgrader;
...
class Admin {
	...
	public function enqueue_admin_assets() {
		...
		wp_localize_script( 'aesthetica-admin', 'cmsmasters_admin', array(
	...
	public function ajax_hide_admin_notice() {
		if ( ! check_ajax_referer( 'cmsmasters_admin_nonce', 'nonce' ) ) {
			wp_send_json( array(
				'success' => false,
				'code' => 'invalid_nonce',
				'message' => esc_html__( 'Invalid nonce. Notice was not deleted.', 'aesthetica' ),
			) );
		}
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json( array(
				...
				'message' => esc_html__( 'You don\'t have permissions.', 'aesthetica' ),
			) );
		}
		...
		if ( ! isset( $_POST['option_key'] ) ) {
			wp_send_json( array(
				...
				'message' => esc_html__( 'Empty option key.', 'aesthetica' ),
			) );
		}
	...
	public function elementor_activation_notice() {
		...
		if ( isset( $plugins['elementor/elementor.php'] ) ) {
			...
			$link_text = esc_html__( 'Activate', 'aesthetica' );
		} else {
			$link_url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=elementor' ), 'install-plugin_elementor' );
			$link_text = esc_html__( 'Install', 'aesthetica' );
		}

		echo '<div class="notice notice-error">' .
			'<p>' .
				sprintf(
					esc_html__( '%s requires Elementor to be activate.', 'aesthetica' ),
					'<strong>' . esc_html__( 'The Theme', 'aesthetica' ) . '</strong>'
				) .
	...
	public function license_activation_notice() {
		...
		echo '<div class="notice notice-warning is-dismissible">' .
			'<p><strong>' . esc_html__( 'Your license is not activated.', 'aesthetica' ) . '</strong></p>' .
			'<p>' .
				sprintf(
					esc_html__(
					...
					'<strong><a href="' . esc_url( self_admin_url( 'admin.php?page=cmsmasters-options-license' ) ) . '">' . esc_html__( 'activate the license', 'aesthetica' ) . '</a></strong>'
				) .
			'</p>' .
		'</div>';
	}
		...
		echo '<div class="notice notice-info is-dismissible cmsmasters-dismiss-notice-permanent" data-option-key="cmsmasters_license_deactivation_notice_visibility">' .
			'<p>' .
				sprintf(
					...
					'<a href="' . esc_url( self_admin_url( 'admin.php?page=cmsmasters-options-license' ) ) . '">' . esc_html__( 'deactivate theme license', 'aesthetica' ) . '</a>'
				) .
			'</p>' .
		'</div>';
	}
	...
	public function apply_demo_notice() {
		...
		echo '<div class="notice notice-info is-dismissible cmsmasters-dismiss-notice-permanent" data-option-key="cmsmasters_apply_demo_notice_visibility">' .
			'<p>' .
				sprintf(
					__( 'You have applied a new design concept to your website. Image sizes in design concepts may differ, this is why it is recommended to run a %1$sRegenerate Thumbnails%2$s tool to generate new image sizes.', 'aesthetica' ),
	...
	public function invalid_admin_email_notice() {
		...
		echo '<div class="notice notice-warning is-dismissible">' .
			'<p>' .
				sprintf(
					__( 'Oops, looks like you tried to use our email: %s', 'aesthetica' ),
					'<strong>' . $current_user->user_email . '</strong>'
				) .
			'</p>' .
			'<p>' . esc_html__( 'Please enter yours instead.', 'aesthetica' ) . '</p>' .
		'</div>';
	}
	...
	public function export_theme_options_name() {
		return 'cmsmasters_aesthetica_options';
	}
