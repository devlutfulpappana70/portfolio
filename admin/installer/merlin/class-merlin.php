<?php
namespace AestheticaSpace\Admin\Installer\Merlin;

use AestheticaSpace\Admin\Installer\Merlin\Includes\WpContentImporter\Class_Merlin_Importer;
use AestheticaSpace\Admin\Installer\Merlin\Includes\Merlin_Customizer_Importer;
use AestheticaSpace\Admin\Installer\Merlin\Includes\Merlin_Downloader;
use AestheticaSpace\Admin\Installer\Merlin\Includes\Merlin_Hooks;
use AestheticaSpace\Admin\Installer\Merlin\Includes\Merlin_Widget_Importer;
use AestheticaSpace\Admin\Installer\Merlin\Merlin_Utils;
...
class Class_Merlin {
	...
	public function init_actions() {
		...
		if (
			...
			delete_option( 'cmsmasters_aesthetica_installer_type' );
			delete_option( 'cmsmasters_aesthetica_content_import' );
		}

		if ( 'ready' === $current_step ) {
			$demo = Utils::get_demo();

			delete_option( "cmsmasters_aesthetica_{$demo}_content_import_files" );

			do_action( 'cmsmasters_import_ready' );

			if ( false === get_option( "cmsmasters_aesthetica_{$demo}_content_import_status" ) ) {
	...
	protected function enqueue_assets() {
		...
		wp_localize_script(
			'aesthetica-installer', 'installer_params', array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'type' => get_option( 'cmsmasters_aesthetica_installer_type' ),
				'content_import' => get_option( 'cmsmasters_aesthetica_content_import' ),
				'wpnonce' => wp_create_nonce( 'cmsmasters_aesthetica_installer_nonce' ),
			)
		);
	}
	...
	public function run_installer() {
		...
		update_option( 'cmsmasters_aesthetica_installer_type', $type, false );

		update_option( 'cmsmasters_aesthetica_content_import', $content_import, false );
	...
	public function set_demo_content_import_files( $demo_key ) {
		...
		update_option( "cmsmasters_aesthetica_{$demo_key}_content_import_files", $data, false );
	}
	...
	public function set_import_files( $files ) {
		$import_files = get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_content_import_files' );
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'aesthetica' ),
		);
	...
	public function steps() {
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'aesthetica' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'aesthetica' ),
				'view' => array( $this, 'license' ),
			);
		}

		$this->steps['demos'] = array(
			'name' => esc_html__( 'Demos', 'aesthetica' ),
			'view' => array( $this, 'demos' ),
		);

		$this->steps['child'] = array(
			'name' => esc_html__( 'Child', 'aesthetica' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'aesthetica' ),
				'view' => array( $this, 'plugins' ),
			);
		}

		// Show the content importer, only if there's demo content added.
		if ( ! empty( $this->import_files ) && 'disabled' !== get_option( 'cmsmasters_aesthetica_content_import' ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'aesthetica' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'aesthetica' ),
	...
	protected function welcome() {
		...
		Logger::debug( __( 'The welcome step has been displayed', 'aesthetica' ) );
	}
	...
	protected function license() {
		...
		echo '<div class="merlin__content--transition">' .
			...
			if ( $is_theme_registered ) {
				echo '<p class="cmsmasters-merlin-license__registered-notice">' . esc_html__( 'The theme is already registered, so you can go to the next step!', 'aesthetica' ) . '</p>';
			}
				...
				echo '<div class="cmsmasters-merlin-license">';
					...
					if ( 'envato-elements' === Theme_Config::MARKETPLACE ) {
						echo '<div class="cmsmasters-merlin-license__source-code">
							<label>
								...
								<span>' . esc_html__( 'I bought the theme on Themeforest', 'aesthetica' ) . '</span>
							</label>
							<label>
								<input type="radio" name="cmsmasters_merlin_license__source_code" value="envato-elements-token" />
								<span>' . esc_html__( 'I downloaded the theme from Envato Elements', 'aesthetica' ) . '</span>
							</label>
						</div>';
					}
					...
					echo '<div class="cmsmasters-merlin-license__code cmsmasters-merlin-license--purchase-code">
						<div class="cmsmasters-merlin-license__code-wrapper">
							<input type="text" name="cmsmasters_merlin_license__purchase_code" placeholder="' . esc_attr__( 'Enter Your Purchase code', 'aesthetica' ) . '" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
						</div>';
					...
					if ( 'envato-elements' === Theme_Config::MARKETPLACE ) {
						echo '<div class="cmsmasters-merlin-license__code cmsmasters-merlin-license--envato-elements-token">
							<span class="cmsmasters-merlin-license__code-description cmsmasters-merlin-license__code-description-top">' .
								sprintf(
									esc_html__( 'In order to activate the theme you need to %1$s', 'aesthetica' ),
									'<a href="' . esc_url( 'https://api.extensions.envato.com/extensions/begin_activation?extension_id=cmsmasters-envato-elements&extension_type=envato-wordpress&extension_description=' . wp_get_theme()->get( 'Name' ) . ' (' . get_home_url() . ')&utm_content=settings' ) . '" target="_blank">' .
										esc_html__( 'generate Envato Elements token', 'aesthetica' ) .
									'</a>'
								) .
							'</span>
							<div class="cmsmasters-merlin-license__code-wrapper">
								<input type="text" name="cmsmasters_merlin_license__envato_elements_token" placeholder="' . esc_attr__( 'Envato Elements Token', 'aesthetica' ) . '" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
					...
					if ( 'templatemonster' !== Theme_Config::MARKETPLACE ) {
						echo '<div class="cmsmasters-merlin-license__user-info">
							<h3 class="cmsmasters-merlin-license__user-info--title">' . esc_html__( 'Register your copy', 'aesthetica' ) . '</h3>
							<p class="cmsmasters-merlin-license__user-info--text">' . esc_html__( 'Get information about promotions, new themes and theme updates directly to your inbox', 'aesthetica' ) . '</p>
							<div class="cmsmasters-merlin-license__user-info--item">
								<input type="text" name="cmsmasters_merlin_license__user_name" placeholder="' . esc_attr__( 'Your Name', 'aesthetica' ) . '" />
							</div>
							<div class="cmsmasters-merlin-license__user-info--item">
								<input type="text" name="cmsmasters_merlin_license__user_email" placeholder="' . esc_attr__( 'Your Email', 'aesthetica' ) . '" />
							</div>
		...
		Logger::debug( __( 'The license activation step has been displayed', 'aesthetica' ) );
	}
	...
	protected function demos() {
				...
				foreach ( $this->demos_list as $demo_key => $demo_args ) {
					...
					echo '<li class="' . esc_attr( $parent_class ) . '__item">' .
						'<figure class="' . esc_attr( $parent_class ) . '__item-image">' .
							...
							( $preview_url ? '<a href="' . esc_url( $preview_url ) . '" target="_blank" class="' . esc_attr( $parent_class ) . '__item-preview"><span title="' . esc_attr( $name ) . '">' . esc_html__( 'Demo Preview', 'aesthetica' ) . '</span></a>' : '' ) .
						'</figure>' .
						'<div class="' . esc_attr( $parent_class ) . '__item-info">' .
							...
							'<div class="' . esc_attr( $parent_class ) . '__item-buttons">' .
								'<a href="' . esc_url( $this->step_next_link() ) . '" class="cmsmasters-install-button cmsmasters-custom" data-key="' . esc_attr( $demo_key ) . '">' . esc_html__( 'Manual', 'aesthetica' ) . '</a>' .
								'<div class="' . esc_attr( $parent_class ) . '__item-buttons-express-wrap">' .
									'<label>' .
										esc_html__( 'Import dummy content?', 'aesthetica' ) .
										'<input type="checkbox" checked="checked" class="cmsmasters-import-content-status" />' .
									'</label>' .
									'<a href="' . esc_url( $this->step_next_link() ) . '" class="cmsmasters-install-button cmsmasters-express" data-key="' . esc_attr( $demo_key ) . '">' . esc_html__( 'One-click Install', 'aesthetica' ) . '</a>' .
								'</div>' .
							'</div>' .
						'</div>' .
					'</li>';
		...
		update_option( 'cmsmasters_aesthetica_installation_status', 'run' );
	}
	...
	public function get_demos_notice() {
		...
		$notice_content .= Merlin_Utils::get_notice_text( esc_html__( 'Your theme provides demo content for a ready website, including all pages, post types, templates and other elements, so in order for it to be installed please make sure your server has appropriate settings:', 'aesthetica' ) );

		if ( ! empty( $limits_to_increase ) ) {
			$notice_content .= Merlin_Utils::get_notice_title( esc_html__( 'Increase the PHP configuration limits to at least:', 'aesthetica' ) );

			$notice_content .= Merlin_Utils::get_notice_list( $limits_to_increase, 'grouped' );
		}

		if ( ! empty( $php_modules_to_include ) ) {
			$notice_content .= Merlin_Utils::get_notice_title( esc_html__( 'Enable PHP modules:', 'aesthetica' ) );
		...
		$notice_content .= Merlin_Utils::get_notice_info( sprintf(
			esc_html__( 'You can find more information %s', 'aesthetica' ),
			'<a href="https://docs.cmsmasters.net/requirements/" target="_blank">' . esc_html__( 'here', 'aesthetica' ) . '</a>'
		) );
	...
	protected function child() {
		...
		Logger::debug( __( 'The child theme installation step has been displayed', 'aesthetica' ) );
	}
	...
	protected function plugins() {
		...
		echo '<div class="cmsmasters-installer-exiting-step-message">
			<p>' . esc_html__( 'Please do not reload or leave the page, the import is still in progress. This will take some time, please wait.', 'aesthetica' ) . '</p>
		</div>';
						...
						foreach ( $required_plugins as $slug => $plugin ) {
							echo '<li data-slug="' . esc_attr( $slug ) . '">
								...
								<label for="default_plugins_' . esc_attr( $slug ) . '">
									...
									<span class="badge">
										<span class="hint--top" aria-label="' . esc_attr__( 'Required', 'aesthetica' ) . '">' .
											esc_html__( 'req', 'aesthetica' ) .
										'</span>
									</span>
								</label>
							</li>';
		...
		Logger::debug( __( 'The plugin installation step has been displayed', 'aesthetica' ) );
	}
	...
	protected function content() {
			...
			if ( 1 < count( $this->import_files ) ) {
				echo '<div class="merlin__select-control-wrapper">' .
					...
					'<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="' . esc_attr__( 'Select Demo', 'aesthetica' ) . '">' .
		...
		Logger::debug( __( 'The content import step has been displayed', 'aesthetica' ) );
	}
	...
	protected function ready() {
		if ( class_exists( 'ElementorPro\Plugin' ) ) {
			...
			$notice_content .= Merlin_Utils::get_notice_title( esc_html__( 'Please Note:', 'aesthetica' ) );

			$notice_content .= Merlin_Utils::get_notice_text( esc_html__( 'CMSmasters Elementor Addon and Elementor Pro are both very powerful plugins that work seamlessly together. However, they both provide the Template Builder functionality, and to make your website look exactly like the demo one after import, CMSmasters Elementor Addon templates are prioritized by default.', 'aesthetica' ) );

			$notice_content .= Merlin_Utils::get_notice_info( sprintf(
				esc_html__( 'You can change the priority at any time by adjusting your settings %1$s.', 'aesthetica' ),
				'<a href="' . admin_url( 'admin.php?page=cmsmasters-addon-settings#tab-pro' ) . '" target="_blank">' . esc_html__( 'here', 'aesthetica' ) . '</a>'
			) );

			$notice_content .= Merlin_Utils::get_notice_info( sprintf(
				esc_html__( 'More information about template priority can be found %1$s.', 'aesthetica' ),
				'<a href="https://docs.cmsmasters.net/how-to-manage-template-priority-between-cmsmasters-elementor-addon-and-elementor-pro/" target="_blank">' . esc_html__( 'here', 'aesthetica' ) . '</a>'
			) );

			$notice_content .= Merlin_Utils::get_notice_button( array(
				'tag' => 'button',
				'text' => esc_html__( 'I understand', 'aesthetica' ),
		...
		Logger::debug( __( 'The final step has been displayed', 'aesthetica' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			Logger::debug( __( 'The existing child theme was activated', 'aesthetica' ) );
		...
		Logger::debug( __( 'The newly generated child theme was activated', 'aesthetica' ) );
	...
	public function ajax_activate_license() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'aesthetica' ),
				)
			);
		}
		...
		if ( ! empty( $error_code ) ) {
			wp_send_json(
				array(
					...
					'message' => esc_html__( 'License key field is empty', 'aesthetica' ),
				)
			);
		}
	...
	public function generate_child_functions_php( $slug ) {
		...
		Logger::debug( __( 'The child theme functions.php content was generated', 'aesthetica' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		Logger::debug( __( 'The child theme style.css content was generated', 'aesthetica' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			Logger::debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'aesthetica' ), array( 'copied' => $copied ) );
		} else {
			Logger::debug( __( 'The child theme screenshot was not generated, because of these results', 'aesthetica' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	public function ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'aesthetica' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'aesthetica' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'aesthetica' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			Logger::debug(
				__( 'A plugin with the following data will be processed', 'aesthetica' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'aesthetica' );
			wp_send_json( $json );
		} else {
			Logger::debug(
				__( 'A plugin with the following data was processed', 'aesthetica' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'aesthetica' ),
				)
			);
		}
	...
	public function ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			Logger::error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'aesthetica' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'aesthetica' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				Logger::info(
					__( 'The content import AJAX call will be executed with this import data', 'aesthetica' ),
					array(
		...
		if ( $json ) {
			...
			Logger::error(
				__( 'The content import AJAX call failed with this passed data', 'aesthetica' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'aesthetica' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			Logger::error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'aesthetica' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'aesthetica' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'aesthetica' ),
				'description'      => esc_html__( 'Demo content data.', 'aesthetica' ),
				'pending'          => esc_html__( 'Pending', 'aesthetica' ),
				'installing'       => esc_html__( 'Installing', 'aesthetica' ),
				'success'          => esc_html__( 'Success', 'aesthetica' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'aesthetica' ),
				'description'      => esc_html__( 'Sample widgets data.', 'aesthetica' ),
				'pending'          => esc_html__( 'Pending', 'aesthetica' ),
				'installing'       => esc_html__( 'Installing', 'aesthetica' ),
				'success'          => esc_html__( 'Success', 'aesthetica' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'aesthetica' ),
				'description'      => esc_html__( 'Sample theme options data.', 'aesthetica' ),
				'pending'          => esc_html__( 'Pending', 'aesthetica' ),
				'installing'       => esc_html__( 'Installing', 'aesthetica' ),
				'success'          => esc_html__( 'Success', 'aesthetica' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'aesthetica' ),
				'description'      => esc_html__( 'After import setup.', 'aesthetica' ),
				'pending'          => esc_html__( 'Pending', 'aesthetica' ),
				'installing'       => esc_html__( 'Installing', 'aesthetica' ),
				'success'          => esc_html__( 'Success', 'aesthetica' ),
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message'] = esc_html__( 'Installing', 'aesthetica' );
		$data['proceed'] = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( ! empty( $homepage->post ) && ! empty( $homepage->post->ID ) ) {
			...
			Logger::debug( __( 'The home page was set', 'aesthetica' ), array( 'homepage_id' => $homepage->post->ID ) );
		}
		...
		if ( ! empty( $blogpage->post ) && ! empty( $blogpage->post->ID ) ) {
			...
			Logger::debug( __( 'The blog page was set', 'aesthetica' ), array( 'blog_page_id' => $blogpage->post->ID ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world->post ) ) {
			...
			Logger::debug( __( 'The Hello world post status was set to draft', 'aesthetica' ) );
		}
		...
		if ( ! empty( $sample_page->post ) ) {
			...
			Logger::debug( __( 'The Sample Page post status was set to draft', 'aesthetica' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				Logger::warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'aesthetica' ), $import_file );
			}
		}
	...
	public function get_import_steps_html( $import_info ) {
		...
		foreach ( $import_info as $slug => $available ) {
			...
			if ( 'content' === $slug ) {
				$item_text = __( 'Dummy Content', 'aesthetica' );
			} elseif ( 'widgets' === $slug ) {
				$item_text = __( 'Sidebars Widgets', 'aesthetica' );
			} elseif ( 'options' === $slug ) {
				$item_text = __( 'Customizer Settings', 'aesthetica' );
			} else {
