...
use AestheticaSpace\Admin\Installer\Merlin\Includes\WpContentImporter\WXR_Importer;
...
class Class_Merlin_Importer extends WXR_Importer {
	...
	protected $cmsmasters_temp_folder_path = 'cmsmasters/aesthetica/import';
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			Logger::critical( __( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'aesthetica' ) );
		...
		if ( ! $status ) {
			Logger::error( __( 'Could not open the XML file for parsing!', 'aesthetica' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( $this->cmsmasters_set_import_media_data() ) {
			if ( ! $this->cmsmasters_import_media() ) {
				Logger::error( __( 'Import Media: Something went wrong. Import stopped.', 'aesthetica' ) );
		...
		if ( is_wp_error( $result ) ) {
			Logger::error( __( 'Content import start error: ', 'aesthetica' ) . $result->get_error_message() );
	...
	public function cmsmasters_set_import_media_data() {
		$content_import_files = get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_content_import_files', array() );

		if ( empty( $content_import_files[0] ) || empty( $content_import_files[0]['import_media_data_file_url'] ) ) {
			return false;
		}

		$this->cmsmasters_content_import_files = $content_import_files[0];
		...
		if ( ! file_exists( $media_data_file_path ) ) {
			$media_data_file_path = File_Manager::download_temp_file( $this->cmsmasters_content_import_files['import_media_data_file_url'], 'media-data.json', $this->cmsmasters_temp_folder_path );
		}

		$all_media_data = File_Manager::get_file_contents( $media_data_file_path );

		if ( empty( $all_media_data ) ) {
			Logger::error( __( 'Import Media: Empty media data.', 'aesthetica' ) );
		...
		if (
			...
			Logger::error( __( 'Import Media: Invalid media data format.', 'aesthetica' ) );
	...
	public function cmsmasters_import_media() {
		if ( 'done' === get_option( 'cmsmasters_aesthetica_import_media_status', 'pending' ) ) {
			return true;
		}

		if ( ! class_exists( '\ZipArchive' ) ) {
			Logger::error( __( 'Import Media: PHP Zip extension not loaded.', 'aesthetica' ) );

			return false;
		}

		for ( $i = 1; $i <= intval( $this->cmsmasters_import_media_data['archives_count'] ); $i++ ) {
			$media_archive_temp_path = File_Manager::download_temp_file( $this->cmsmasters_content_import_files['import_files_url'] . "media/media-archive-{$i}.zip", "media-archive-{$i}.zip", $this->cmsmasters_temp_folder_path );

			if ( ! file_exists( $media_archive_temp_path ) ) {
				Logger::error( __( 'Import Media: Unable to download media archive.', 'aesthetica' ) );
			...
			if ( true === $zip->open( $media_archive_temp_path ) ) {
				...
				Logger::info( sprintf( __( 'Import Media: Media archive %d extracted successfully.', 'aesthetica' ), $i ) );
			} else {
				Logger::error( sprintf( __( 'Import Media: Failed to open archive %d.', 'aesthetica' ), $i ) );
			}

			@unlink( $media_archive_temp_path ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
		}

		Logger::info( __( 'Import Media: All media archives processed.', 'aesthetica' ) );

		update_option( 'cmsmasters_aesthetica_import_media_status', 'done' );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			...
			$cmsmasters_imported_attachments_ids = get_option( "cmsmasters_aesthetica_{$cmsmasters_demo}_import_attachments_ids" );

			if ( isset( $cmsmasters_imported_attachments_ids[ $original_id ] ) ) {
				Logger::notice( sprintf(
					__( 'Skipping attachment "%s", this attachment already imported', 'aesthetica' ),
					$data['post_title']
				) );
			...
			if ( is_wp_error( $post_id ) ) {
				Logger::error( __( 'Import Attachment error: ', 'aesthetica' ) . $post_id->get_error_message() );
			...
			$cmsmasters_imported_posts_ids = get_option( "cmsmasters_aesthetica_{$cmsmasters_demo}_import_displayed_ids" );

			if ( isset( $cmsmasters_imported_posts_ids[ $original_id ] ) ) {
				Logger::notice( sprintf(
					__( 'Skipping post "%s", this post already imported', 'aesthetica' ),
					$data['post_title']
				) );
	...
	protected function cmsmasters_process_attachment( $post, $meta, $import_media_data = array() ) {
		if ( empty( $import_media_data ) ) {
			return new \WP_Error( 'attachment_processing_error', __( 'Media data not found for original ID.', 'aesthetica' ) );
		}

		$file_path = $this->cmsmasters_wp_upload_basedir . $import_media_data['relative_path'];

		if ( ! file_exists( $file_path ) ) {
			return new \WP_Error( 'attachment_processing_error', sprintf( __( 'File not found in uploads directory: %s', 'aesthetica' ), $file_path ) );
		}
		...
		if ( ! empty( $attachment_metadata ) ) {
			...
			Logger::warning( sprintf( __( 'Attachment metadata not found for post ID %d.', 'aesthetica' ), $post_id ) );
		}
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 25 ) ) {
			...
			Logger::info( __( 'New AJAX call!', 'aesthetica' ) );
