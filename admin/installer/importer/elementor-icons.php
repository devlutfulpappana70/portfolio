...
class Elementor_Icons {
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		if ( 'done' === get_option( "cmsmasters_aesthetica_{$demo}_elementor_icons_import" ) ) {
			return;
		}

		update_option( "cmsmasters_aesthetica_{$demo}_elementor_icons_import", $status );
	}
	...
	public function get_local_icons() {
		...
		if ( 'pending' !== get_option( "cmsmasters_aesthetica_{$demo}_elementor_icons_import", 'done' ) ) {
			return;
		}

		$data = API_Requests::post_request( 'get-elementor-icons', array( 'demo' => Utils::get_demo() ) );
		...
		update_option( "cmsmasters_aesthetica_{$demo}_elementor_icons_import", 'done' );
	}
	...
	private function import_single_icon( $file_path ) {
		$post_id = wp_insert_post( wp_slash( array(
			...
			'post_title' => esc_html__( '(no title)', 'aesthetica' ),
			'post_status' => 'draft',
		) ), true );
	...
	public function local_icon_upload_handler( $data ) {
		...
		if ( empty( $config ) ) {
			return new \WP_Error( 'unsupported_zip_format', esc_html__( 'The zip file provided is not supported!', 'aesthetica' ) );
		}
