...
class Elementor_Importer {
	...
	public function end_import() {
		Logger::info( __( 'Start import remapping ids process', 'aesthetica' ) );
		...
		update_option( "cmsmasters_aesthetica_{$demo}_content_import_status", 'imported', false );

		Logger::info( __( 'Finish import remapping process', 'aesthetica' ) );
	}
	...
	public function change_import_elements_ids() {
		...
		Logger::info( __( 'Starting the process of remapping the IDs of imported data', 'aesthetica' ) );

		$demo = Utils::get_demo();

		$patterns = array();

		$templates_ids = get_option( "cmsmasters_aesthetica_{$demo}_elementor_import_templates_ids", array() );
		...
		$attachments_ids = get_option( "cmsmasters_aesthetica_{$demo}_import_attachments_ids", array() );
				...
				'replacement' => function ( $matches ) use ( $attachments_ids, $wp_uploads_dir ) {
					...
					if ( ! is_array( $match ) || ! isset( $match['id'] ) || ! isset( $match['url'] ) ) {
						Logger::debug( sprintf(
							__( 'Invalid JSON structure: %1$s', 'aesthetica' ),
							$matches[0]
						) );
		...
		foreach ( $post_ids as $post_id ) {
			...
			Logger::info( sprintf(
				__( 'Start remapping imported data in %1$s %2$d', 'aesthetica' ),
				$post_type,
				$post_id,
			) );
			...
			Logger::info( sprintf(
				__( 'Finish remapping imported data in %1$s %2$d', 'aesthetica' ),
				$post_type,
				$post_id,
			) );
		}

		Logger::info( __( 'Finishing the process of remapping the IDs of imported data', 'aesthetica' ) );
	}
	...
	protected function change_megamenu_import_templates_ids() {
		...
		Logger::info( __( 'Starting the process of remapping templates ids in mega menu items', 'aesthetica' ) );
		...
		foreach ( $menus as $menu ) {
			...
			Logger::info( sprintf(
				__( 'Start remapping imported data in menu %1$s %2$d', 'aesthetica' ),
				$menu->name,
				$menu->term_id,
			) );
			...
			Logger::info( sprintf(
				__( 'Finish remapping imported data in menu %1$s %2$d', 'aesthetica' ),
				$menu->name,
				$menu->term_id,
			) );
		}

		Logger::info( __( 'Finishing the process of remapping templates ids in mega menu items', 'aesthetica' ) );
	}
