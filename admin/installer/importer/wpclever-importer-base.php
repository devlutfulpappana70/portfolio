...
class WPClever_Importer_Base {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME . '_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME . '_import', $status );
	}
	...
	protected function set_exists_options() {
		$this->options = get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME, array() );
	}
	...
	protected function set_api_options() {
		...
		$data = API_Requests::post_request( 'get-wpclever-data', array(
			'demo' => Utils::get_demo(),
	...
	public static function set_backup_options( $first_install = false ) {
		...
		$option_name = 'cmsmasters_aesthetica_' . Utils::get_demo() . '_wpclever_' . self::MODULE_NAME;

		if ( $first_install ) {
			$option_name = 'cmsmasters_aesthetica_wpclever_' . self::MODULE_NAME . '_backup';
		}
