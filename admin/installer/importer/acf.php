...
class ACF extends Importer_Base {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_aesthetica_acf_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		update_option( 'cmsmasters_aesthetica_acf_import', $status );
	}
	...
	protected function set_exists_options() {
		$this->options = get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_acf', array() );
	}
	...
	protected function set_api_options() {
		...
		$data = API_Requests::post_request( 'get-acf-settings', array( 'demo' => Utils::get_demo() ) );
	...
	public static function set_backup_options( $first_install = false ) {
		...
		$option_name = 'cmsmasters_aesthetica_' . Utils::get_demo() . '_acf';

		if ( $first_install ) {
			$option_name = 'cmsmasters_aesthetica_acf_backup';
		}
