...
class Revslider {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_revslider_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_aesthetica_' . Utils::get_demo() . '_revslider_import', $status );
	}
	...
	protected function import_slides() {
		$data = API_Requests::post_request( 'get-revslider', array( 'demo' => Utils::get_demo() ) );
